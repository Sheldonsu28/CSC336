...
def calculate_energy_diff(xp, yp, xv, yv, m):
    ...
    for i in range(0, 9):
        for j in range(i + 1, 9):
            inital_r = np.sqrt(((xp[i, 0] - xp[j, 0])**2) + ((yp[i, 0] - yp[j, 0])**2))
            final_r = np.sqrt(((xp[i, -1] - xp[j, -1])**2) + ((yp[i, -1] - yp[j, -1])**2))

            inital_U += G*m[i]*m[j]/inital_r
            Final_U += G*m[i]*m[j]/final_r
...
def simulation_Runge_kutta_2rd(simulation_time, dt):
    """
    Simulate planetory motion using Velocity verlet methods
    :param simulation_time: Total simulation time (in seconds)
    :param dt: time step (in seconds)
    ...
    entity_mass = np.array(
        [1988500e24, 0.33011e24, 4.8675e24, 5.9723e24, 0.64171e24, 1898.19e24, 568.34e24, 86.813e24,
         102.413e24], dtype=np.longdouble)
    ...
    for i in range(0, simulation_length):
        ...
        for p in range(1, 9):
            ...
            k2_xa, k2_ya = calc_acc(entity_position_x[p, i] + dt * k1_xv, entity_position_x[:, i],
                                    entity_position_y[p, i] + dt * k1_yv,
                                    entity_position_y[:, i], p, entity_mass)

            k2_xv = entity_velocity_x[p, i] + dt * k1_xa
            k2_yv = entity_velocity_y[p, i] + dt * k1_ya


            entity_position_x[p, i + 1] = entity_position_x[p, i] + (dt / 2) * (k1_xv + k2_xv)
            entity_position_y[p, i + 1] = entity_position_y[p, i] + (dt / 2) * (k1_yv + k2_yv)

            entity_velocity_x[p, i + 1] = k1_xv + (dt / 2) * (k1_xa + k2_xa)
            entity_velocity_y[p, i + 1] = k1_yv + (dt / 2) * (k1_ya + k2_ya)

    c = time.time()
    print("RK2 simulation took:", c - a, "s to finish")
    ...
    plt.xlabel("distance, unit: m")
    plt.ylabel("distance, unit: m")
    plt.savefig("RK2 {}.png".format(simulation_time / 3.154e+7))
